...
- **启动函数**: `main()` - 使用 `#[someboot::entry]` 宏标记的平台主函数

### 启动流程
```rust
#[someboot::entry]
fn main() -> ! {
    someboot::println!("Starting Sparreal OS kernel...");
...
```toml
[dependencies]
someboot = { workspace = true }     # 硬件抽象层
sparreal-kernel = { workspace = true }  # 内核核心
log = {workspace = true}           # 日志接口
```

### 特性配置
- **`hv`**: 虚拟化支持（传递给 someboot）
- **`uspace`**: 用户空间支持（传递给 someboot，依赖 mmu）
