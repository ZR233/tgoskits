...
- **Axvisor guest platform identity**: every Axvisor guest machine has exactly one machine-owned virtual UART; guest TOML must not select its model, address, IRQ, backend, or enabled state. On firmware-backed hosts, replacing a physical platform device with an emulated implementation must preserve the firmware-visible identity: derive the virtual UART node path, phandle, address span, register model/shift/access width, interrupt-parent, and IRQ specifier from FDT `stdout-path`, and place the vGIC distributor/redistributor at the host GIC windows while retaining its phandle. Firmware may describe padded, mutually overlapping GICv2 ranges; retain their bases but normalize the trapped guest apertures to the architectural 4 KiB Distributor and 8 KiB CPU-interface spans before device-resource registration, rather than weakening generic overlap validation. Keep architecture profile values as the fallback for fixed machines such as QEMU. The corresponding physical UART and GIC ranges remain host-owned and must be removed from both explicit and default device-passthrough mapping even though the virtual devices trap the same guest addresses. Axvisor's application-layer console mux is the sole host-console input reader. Its `SerialBackendFactory` must create a fresh per-device-generation backend whenever the VM device graph is rebuilt; stop/delete invalidates the old generation so retained callbacks cannot read from or write to a replacement VM. A host-derived UART INTID remains a virtual IRQ line routed through the guest controller; matching the number does not enable physical IRQ passthrough.
- **Axvisor shared firmware providers**: replacing a host-owned FDT device also removes that device as a visible consumer of shared clocks, resets, power domains, or other providers. A passthrough guest must not retain unmediated write access that can disable the host-owned dependency. Preserve provider phandle/specifier/MMIO identity in the immutable machine plan, resolve a typed provider capability before VM construction, and let a `DeviceRuntime` factory claim the whole provider range so stage 2 traps it. The device may forward unrelated accesses only under provider-supplied, hardware-specific write-protection rules; the VM hot path must not contain board or SoC checks. Fixed providers without mutable registers need no mediator. Missing capability, ambiguous register layout, unsupported specifier shape, or invalid protection rules must fail VM creation instead of falling back to raw passthrough. Keep this contract synchronized with `book/design/axvisor-shared-firmware-provider.md`.
